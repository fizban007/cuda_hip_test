...
__global__ void generic_kernel(Func f, Args... args) {
  f(args...);
}
...
int main(int argc, char *argv[]) {
  ...
  generic_kernel<<<512, 512>>>(
      [N, N_grid] __device__(auto ptc_w, auto ptc_cell, auto grid_w) {
          int tid = threadIdx.x + blockIdx.x * blockDim.x;

          for (int n = tid; n < N; n += blockDim.x * gridDim.x) {
            atomicAdd(&grid_w[ptc_cell[n]], ptc_w[n]);
          }
      },
      p_ptc_w, p_ptc_cell, p_grid_w);
